...
def linearRegression():
    ...
    text.insert(END,"Linear Regression Insulin Dosage Prediction Accuracy : "+str(lr_acc)+"\n\n")
...
def predict():
    ...
    for i in range(len(diabetes_test)):
        if diabetes_prediction[i] == 0:
            text.insert(END, 'No Diabetes Detected'+"\n\n")
        if diabetes_prediction[i] == 1:
            text.insert(END, 'Diabetes Detected & Required Insulin Dosage : '+str(int(insulin_prediction[i]))+"\n\n")
...
lrButton = Button(main, text="Run Linear Regression Algorithm", command=linearRegression)
